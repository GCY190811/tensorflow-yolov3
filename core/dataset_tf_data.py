...
def data_func(trainPattern, valPattern):
    train_dataset = tf.data.TFRecordDataset(filenames=tf.gfile.Glob(trainPattern))
    train_dataset = train_dataset.map(parse_func("train"), num_parallel_calls=6)
    train_dataset = train_dataset.repeat(cfg.TRAIN.WARMUP_EPOCHS+cfg.TRAIN.FIRST_STAGE EPOCHS+cfg.TRAIN.SECOND_STAGE_EPOCHS)  # repeat 如何处理
    ...
    val_dataset = val_dataset.repeat(cfg.TRAIN.WARMUP_EPOCHS+cfg.TRAIN.FIRST_STAGE EPOCHS+cfg.TRAIN.SECOND_STAGE_EPOCHS)  # repeat 如何处理
...
def parse_func(mode, serialized_example):
    features = tf.parse_single_example(
        ...
        features = {
            ...
            'bboxes': tf.FixedLenFeature([], tf.strin),
...
def preprocess(mode, image, true_boxes, true_labels):
    ...
    label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes = bboxes_preprocess(bboxes, output_sizes, num_classes)

    batch_image = np.zeros((1, input_size, input_size, 3))
    batch_label_sbbox = np.zeros((1, output_sizes[0], output_sizes[0],
                                  anchor_per_scale, 5 + num_classes))
    batch_label_mbbox = np.zeros((1, output_sizes[1], output_sizes[1],
                                  anchor_per_scale, 5 + num_classes))
    batch_label_lbbox = np.zeros((1, output_sizes[2], output_sizes[2],
                                  anchor_per_scale, 5 + num_classes))
    batch_sbboxes = np.zeros((1, max_bbox_per_scale, 4))
    batch_mbboxes = np.zeros((1, max_bbox_per_scale, 4))
    batch_lbboxes = np.zeros((1, max_bbox_per_scale, 4))
...
def image_preprocess(image, bboxes, input_size):
    ...
    image, bboxes = img.letterBox(np.copy(image),
                                  [input_size, input_size], np.copy(bboxes))
    return image, bboxes


def bboxes_preprocess(bboxes, output_sizes, num_classes):
    '''
    input:
    ...
    output:
        计算loss中使用的bbox
    '''
    label = [np.zeros((output_sizes[i], output_sizes[i],
